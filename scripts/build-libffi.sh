#!/bin/bash
# build-libffi.sh - Build libffi for Android
...
if [ ! -f configure ]; then
  ...
  log_cmd ./autogen.sh || log_cmd autoreconf -fi
fi
...
# libffi builds in a subdirectory named after the target
# Find the actual build directory created by configure
BUILD_SUBDIR=$(find . -maxdepth 1 -type d -name "*linux-android*" | head -1)
if [ -z "$BUILD_SUBDIR" ]; then
  log "ERROR: Could not find libffi build directory"
  exit 1
fi

log "Found build directory: $BUILD_SUBDIR"
cd "$BUILD_SUBDIR"

# Patch Makefile to skip doc directory
sed -i 's/SUBDIRS = include testsuite man doc/SUBDIRS = include testsuite man/' Makefile
