#!/bin/bash
# build-qcom-libqrtr-glib.sh - Build Qualcomm QRTR GLib library for Android
...
SRC_DIR="$BUILD_DIR/src/libqrtr-glib-src"
...
if [ -d "$SCRIPT_DIR/../src/qcom-libqrtr-glib" ] && [ ! -d "$SRC_DIR" ]; then
  log "Using local qcom-libqrtr-glib source..."
  mkdir -p "$BUILD_DIR/src"
  cp -r "$SCRIPT_DIR/../src/qcom-libqrtr-glib" "$SRC_DIR"
else
...
# Set up PKG_CONFIG_PATH for glib2
export PKG_CONFIG_PATH="$PREFIX/glib2/lib/pkgconfig:${PKG_CONFIG_PATH:-}"
...
[built-in options]
c_args = ['-I$PREFIX/glib2/include/glib-2.0', '-I$PREFIX/glib2/lib/glib-2.0/include']
c_link_args = ['-L$PREFIX/glib2/lib']
...
if [ ! -f "$INSTALL_DIR/lib/libqrtr-glib.a" ]; then
  log "ERROR: libqrtr-glib.a not found"
  exit 1
fi
