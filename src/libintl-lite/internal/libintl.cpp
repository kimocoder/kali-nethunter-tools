...
// GLib compatibility aliases
// GLib expects these symbols with g_libintl_ prefix
extern "C" {
	const char* g_libintl_textdomain(const char* domain) {
		return textdomain(domain);
	}
	
	libintl_lite_bool_t g_libintl_bindtextdomain(const char* domain, const char* moFilePath) {
		return bindtextdomain(domain, moFilePath);
	}
	
	libintl_lite_bool_t g_libintl_bind_textdomain_codeset(const char* domain, const char* moFilePath) {
		return bind_textdomain_codeset(domain, moFilePath);
	}
	
	const char* g_libintl_gettext(const char* origStr) {
		return gettext(origStr);
	}
	
	const char* g_libintl_dgettext(const char* domain, const char* origStr) {
		return dgettext(domain, origStr);
	}
	
	const char* g_libintl_dcgettext(const char* domain, const char* origStr, int category) {
		// libintl-lite doesn't support categories, just call dgettext
		(void)category;  // unused
		return dgettext(domain, origStr);
	}
	
	const char* g_libintl_ngettext(const char* origStr, const char* origStrPlural, unsigned long n) {
		return ngettext(origStr, origStrPlural, n);
	}
	
	const char* g_libintl_dngettext(const char* domain, const char* origStr, const char* origStrPlural, unsigned long n) {
		return dngettext(domain, origStr, origStrPlural, n);
	}
	
	const char* g_libintl_dcngettext(const char* domain, const char* origStr, const char* origStrPlural, unsigned long n, int category) {
		// libintl-lite doesn't support categories, just call dngettext
		(void)category;  // unused
		return dngettext(domain, origStr, origStrPlural, n);
	}
}
